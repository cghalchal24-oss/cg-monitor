...
async function scrapeWebsite(site) {
  try {
    const { data } = await axios.get(site.url, { timeout: 5000, headers: { 'User-Agent': 'Mozilla/5.0' } });
...
app.get('/api/updates', async (req, res) => {
  ...
  if (!cache.timestamp || (now - cache.timestamp) > 120000) {
    try {
      ...
      return res.json({ updates: [], pdfs: [], history: [], totalSites: 0 });
    }
  }
  res.json(cache.data);
});
...
// Vercel expects default export as a serverless function
module.exports = app;
